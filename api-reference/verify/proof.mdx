...
<Warning>
  **Illustrative example.** The two-stamp response below uses a `witnesschain` stamp to show what multi-source cross-correlation looks like. WitnessChain is **not yet implemented** — today only `proofmode` (alpha) and `mock` are available, so a real multi-stamp proof would pair `proofmode` with `mock`. Some `details` fields shown here (for example `certificateChainValid`) correspond to checks that are **planned but not yet performed in v0** — see the [ProofMode plugin](/plugins/proofmode) for exactly what verification does today. The credibility-vector structure is also still evolving.
</Warning>
