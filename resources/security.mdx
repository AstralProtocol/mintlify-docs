...
**Status:** Out of scope for the research preview

Astral can verify that a computation was correct, but cannot verify that the input location represents where the user actually was. GPS can be spoofed. Location proofs raise the cost of spoofing but do not make it impossible — the goal is to raise forgery cost above the value of the action a proof supports, not to achieve absolute certainty.
