...
// Stamps module - evidence collection and verification
astral.stamps.collect({ plugins: ['mock'] });
astral.stamps.create({ plugin: 'mock' }, signals);
astral.stamps.sign({ plugin: 'mock' }, unsigned, signer);
astral.stamps.verify(stamp);

// Proofs module - bundle claim + stamps and evaluate credibility
...
│   └── verify()        - Verify a stamp's internal validity
├── proofs: ProofsModule
│   ├── create()        - Bundle claim + stamps into proof
│   └── verify()        - Full proof verification + credibility assessment
```
