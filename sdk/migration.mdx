...
1. **Unified Package**: Location attestations and geospatial computations are now in a single package
2. **Namespaced API**: Methods are organized under `location.offchain.*`, `location.onchain.*`, and `compute.*`
3. **New Configuration**: Unified `AstralConfig` replaces separate configurations
4. **New Compute Module**: Integrated `ComputeModule` replaces `@decentralized-geo/astral-compute`
