...
<Warning>
  **Alpha** — The ProofMode plugin is in early development. Bundle parsing, stamp creation, and `verify()` are available. See below for what's coming next.
</Warning>

# ProofMode plugin

[ProofMode](https://proofmode.org) is a mobile app (iOS and Android) that collects device evidence — GPS coordinates, network context, device metadata, PGP signatures, and SafetyNet/Play Integrity attestations. The `@location-proofs/plugin-proofmode` package parses ProofMode's export bundles into Astral location stamps and verifies their internal consistency.

## How it works today

1. The user captures evidence in the ProofMode app on their phone
2. They export a ZIP bundle from the app
3. Your application passes the ZIP bundle to the plugin
4. The plugin parses it into an `UnsignedLocationStamp` and can verify its structure and signals

```
ProofMode App → ZIP export → plugin.parseBundle() → plugin.createStampFromBundle() → UnsignedLocationStamp
                                                   → plugin.verify() → StampVerificationResult
```
...
The resulting stamp contains:
- Location as a GeoJSON Point from GPS coordinates
- Temporal footprint from device timestamps
- All parsed signals including SafetyNet JWT, PGP key presence, file hashes

<Note>
  ProofMode does **not** implement the standard `create(signals)` interface method. Use `parseBundle()` + `createStampFromBundle()` instead. This means `astral.stamps.create({ plugin: 'proofmode' }, signals)` is not supported — use the plugin's bundle-based workflow directly.
</Note>
...
console.log('Location:', bundle.metadata.signals['Location.Latitude'],
            bundle.metadata.signals['Location.Longitude']);
console.log('Has PGP key:', !!bundle.publicKey);
console.log('Has SafetyNet:', !!bundle.safetyNetToken);
...
// Sign the stamp (with your own key, since ProofMode's sign is handled on-device)
