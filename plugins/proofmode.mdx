...
<Warning>
  **Alpha** — The ProofMode plugin is in early development. The `create()` and `verify()` methods are available. See below for what's coming next.
</Warning>
...
Plugin interface method — wraps `createStampFromBundle()` for use with the `StampsModule`.
