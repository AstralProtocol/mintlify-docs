...
Not every plugin implements every stage. ProofMode, for example, collects evidence on-device via its mobile app — the plugin handles parsing and verification, not collection. WitnessChain collects via an API. The MockPlugin implements all four for testing. Client-side, the SDK ships MockPlugin today and ProofMode is the most developed real source; server-side, the verify service registers verifiers for several systems (see below) at varying maturity.
