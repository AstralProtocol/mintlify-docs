...
Not every plugin implements every stage. ProofMode, for example, collects evidence on-device via its mobile app — the plugin handles parsing and verification, not collection. WitnessChain is designed to collect via an API (the integration is planned, not yet built). The MockPlugin implements all four for testing. Today, ProofMode is the only implemented proof-of-location plugin; Mock is for testing.
