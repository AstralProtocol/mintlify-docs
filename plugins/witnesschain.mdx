...
<Warning>
  **In development** — The WitnessChain **client** plugin (collecting and creating stamps) is under active development on the `develop` branch. The verify service already registers a server-side WitnessChain verifier, so submitted stamps can be evaluated; the maturity of both is still early in this research preview.
</Warning>
