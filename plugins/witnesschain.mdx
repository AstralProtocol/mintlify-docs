...
<Warning>
  **Experimental** — WitnessChain is an early-stage plugin. The interface is defined and the verify service has an experimental server-side verifier; the client plugin (collecting and creating stamps) is still under development on the `develop` branch. ProofMode is the plugin that's working end to end today. We'd love to develop this and other proof-of-location plugins with partners — [get in touch](mailto:contact@astral.global).
</Warning>
