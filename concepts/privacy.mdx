...
Privacy is designed into Astral from the start — not bolted on afterward. A [TEE](/concepts/astral-location-services) running under attestation provides meaningful privacy properties, and zero-knowledge circuits for location proof verification and geospatial computations are an active research direction for stronger properties in the future. The privacy properties below assume the enclave runs under continuous remote attestation; see the [trust model](/trust-model/what-you-are-trusting) for the current deployment status.
