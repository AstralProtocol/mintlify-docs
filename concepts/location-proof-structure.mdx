...
|--------|------------------------|-------------|--------|
| [ProofMode](/plugins/proofmode) | Device attestation + sensor fusion | iOS, Android, React Native | Implemented (alpha) |
| [WitnessChain](/plugins/witnesschain) | Infrastructure latency triangulation | Node.js, any HTTP client | Planned — interface defined, not yet built |
| [Mock](/plugins/mock) | Configurable test evidence | Any JavaScript/TypeScript environment | Available for testing |

<Tip>
  Different devices and environments suit different plugins. A mobile app might use ProofMode for on-device attestation, while a backend service could use a network-based plugin like WitnessChain once it's built. Today, ProofMode is the only implemented proof-of-location source. See [Plugins](/plugins/overview) for details on each.
</Tip>
