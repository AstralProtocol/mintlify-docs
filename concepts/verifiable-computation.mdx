...
1. Location Verification    →    2. Geospatial Computation    →    3. Onchain Verification
   (Where is the user?)           (Policy evaluation)               (Smart contract logic)
      In development               ✓ ASTRAL LOCATION SERVICES           ✓ EAS Resolvers
```
...
Trust is anchored in the TEE boundary:
...
**The computation problem (what Astral Location Services solves):**
...
Location proofs become useful when they can trigger onchain actions through geospatial policies. Astral Location Services applies those policies — checking if a location is inside a boundary, within range of a target, or satisfies other spatial constraints.
