...
v0 of Astral targets these proof-of-location systems through [location proof plugins](/plugins/overview). So far only ProofMode is implemented:
...
<CardGroup cols={2}>
  ...
  <Card title="WitnessChain — planned" icon="server" href="/plugins/witnesschain">
    Infrastructure verification. Intended to use UDP latency triangulation across a distributed challenger network, where trust derives from the speed of light — you can't fake being close to many geographically distributed nodes at once. The plugin interface is defined, but the integration is not yet built.
  </Card>
</CardGroup>
...
<Info>
  Beyond ProofMode, the plugin set is still to be developed. We're [researching](https://github.com/location-proofs) more proof-of-location plugins and would welcome collaborators — if you work on a proof-of-location system, [get in touch](mailto:contact@astral.global).
</Info>
