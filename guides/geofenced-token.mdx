...
# Build a Geofenced Token

Create a local currency that can only be traded by people physically in a specific region — enabling hyperlocal economies.

<Info>
  **About location verification**: This guide uses GPS coordinates as input. GPS is spoofable. As [Location Proof plugins](https://collective.flashbots.net/t/towards-stronger-location-proofs/5323) are developed, they'll replace `getCurrentLocation` for stronger verification.
</Info>

## Concept

Imagine a "San Francisco Token" (SFT) that:
- Can only be minted by SF residents
- Can only be transferred to people currently in SF
- Creates a true local currency for the Bay Area
...
const region = await astral.location.create(sfBoundary, {
  submitOnchain: true,
  metadata: {
    name: "San Francisco",
    description: "SF city boundary for local currency"
  }
});
...
async function mintLocalToken(amount: bigint, wallet: Signer) {
  ...
  const tx2 = await astral.eas.submitDelegated(proof.delegatedAttestation);
  await tx2.wait();
