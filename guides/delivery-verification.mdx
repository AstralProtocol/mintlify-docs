...
<Info>
  **About location verification**: This guide uses GPS coordinates as input. GPS is spoofable. We're working on [Location Proof plugins](https://collective.flashbots.net/t/towards-stronger-location-proofs/5323) that will replace `Geolocation.getCurrentPosition` for stronger verification — these are still in development.
</Info>

## Concept

An evidence-based delivery system where:
- Buyer locks payment in escrow
- Delivery person submits a location check at the destination
- Escrow releases when that check verifies against the destination

(Not "trustless" — you're still trusting the location input and Astral's signer; see the note above. It removes the need to trust the *computation*, not the inputs.)
