...
ProofMode is the plugin that's working end to end today. The verify service also registers experimental verifiers — `witnesschain`, `gpsd`, `geoclue`, `wifi-mls`, and `ip-geolocation` — with interfaces defined. We're keen to develop new ones with partners. Two are highlighted here:
