...
<Info>
  **About location verification**: This guide uses GPS coordinates as input. GPS is spoofable. For stronger verification, see [location proofs](/concepts/location-proofs) — Astral's plugin system combines evidence from device sensors, network infrastructure, and other sources into credibility assessments. See the [creating location proofs](/guides/creating-location-proofs) guide to integrate location proofs into your app.
</Info>
...
<Warning>
  **Schema must use `revocable: true`** — Astral signs delegated attestations with `revocable: true`. If your schema is registered with `revocable: false`, EAS will reject the attestation with an `Irrevocable()` or `InvalidSignature()` error. See [#9](https://github.com/AstralProtocol/mintlify-docs/issues/9) for discussion on this behavior.
</Warning>
...
<Warning>
  **Location source**: The `navigator.geolocation` API provides GPS coordinates which are spoofable. For stronger verification, use [location proof plugins](/concepts/location-proofs) to collect evidence from multiple independent sources.
</Warning>
