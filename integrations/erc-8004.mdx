...
2. **Evidence collection** — The agent uses [location proof plugins](/plugins/overview) to collect location evidence. The design supports many independent signal sources — GPS hardware, WiFi geolocation, IP lookup, device attestation, infrastructure triangulation — behind a common interface. Plugin maturity varies in this research preview: ProofMode is the most developed device-side source, and the verify service registers verifiers for several others (WitnessChain, gpsd, GeoClue, Wi-Fi/MLS, IP geolocation).
...
<CardGroup cols={2}>
  <Card title="Evidence-based delivery" icon="truck">
