...
<Warning>
  **Illustrative pseudocode.** The agent-side API below (`AstralClient`, `collectEvidence`, `generateProof`) is a sketch for the OpenClaw concept and does **not** match the shipping SDK surface. The real SDK is `AstralSDK` from `@decentralized-geo/astral-sdk`, using `stamps.collect(...)` and `proofs.create(...)` — see the [SDK overview](/sdk/overview). Treat this as a design illustration, not a working import.
</Warning>
...
class LocationAwareAgent extends ERC8004Agent {
  ...
  constructor(config: AgentConfig) {
    ...
    this.astral = new AstralClient({
      endpoint: "https://staging-api.astral.global",
      agentId: this.id,
    });
  }
...
Astral cross-correlates multiple signals to produce a [credibility vector](/concepts/location-proof-evaluation) — a multidimensional assessment, not a single score. No single signal is trusted alone. (OpenClaw's on-chain `LocationProof` struct collapses that vector into one `credibility` scalar for cheap on-chain comparison; that scalarization is OpenClaw's design choice, not Astral's native output.)
...
When signals agree, the evidence is stronger; when they disagree, the assessment reflects the inconsistency.

<Note>
  The JSON below is **OpenClaw's own illustration** of reducing signals to a weighted scalar — it is **not** the shape Astral returns. Astral returns a structured credibility vector (spatial, temporal, validity, independence dimensions, currently), with no single `overall` score by design, and that structure is still evolving. See [Location Proof Evaluation](/concepts/location-proof-evaluation).
</Note>
